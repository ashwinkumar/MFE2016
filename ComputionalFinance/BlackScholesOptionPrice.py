...
def N(d):
    ...
    if d >= 0 :
        ...
        return (1- 0.5*np.pow((1+ CONST_D1*d + CONST_D2*d_2 + CONST_D3*d_3 + CONST_D4*d_4 + CONST_D5*d_5 + CONST_D6*d_6),-16))
    if d <0 :
        return(1- N(-d))
    '''
    return norm.cdf(d)
...
def getd1(S0, X, r, sigma, t):
    ...
    return (np.log(S0/X) + (r + sigma*sigma/2) * t )/ (sigma * np.sqrt(t))

def getd2(S0, X, r, sigma, t):
    #return getd1(S0, X, r , sigma, t) - sigma* math.sqrt(t)
    return getd1(S0, X, r, sigma, t) - sigma * np.sqrt(t)
...
def EuropeanCallPrice(S0, X, r , sigma, t):
    ...
    return S0 * N(d1) - X * np.exp(-r * t) * N(d2)


def EuropeanPutPrice(S0, X, r , sigma, t):
    d1 = getd1(S0, X, r, sigma, t)
    d2 = getd2(S0, X, r, sigma, t)
    #return S0* norm.cdf(d1) - X* math.exp(-r*t)* norm.cdf(d2)
    #return X* math.exp(-r*t)* N(-d2) - S0* N(-d1)
    return X* np.exp(-r*t)* N(-d2) - S0* N(-d1)
...
def getGamma(S0,X,r,sigma,t):
    ...
    return n(d1)/(S0*sigma*np.sqrt(t))
